      ...
      if (!single.ageGroup || !single.player || !single.paymentMethod) {
        ...
        if (databaseWrapper.isValidObjectId(single.player)) {
          ...
          if (
            ...
            validateArray.length == 1 &&
            single.pastPerformance == null
          ) {
            return res
              .status(400)
          ...
          if (player.data == null || player.data.length == 0) {
            return res.status(400).send({
              message: 'Invalid Player ID. Register as a player first',
            });
          }
          //check whether the player already registered for single
          const singlePlayer = await databaseWrapper.read(
            'single',
            res,
            ['player'],
            [single.player]
          ...
          if (singlePlayer.data?.length != 0) {
            return res.status(409).send({
              message:
                'This player ID is already registered for a single match',
            });
          }
          ...
          const singleData = {
            ...
            matchType:
              player.data[0].gender == 'Male'
                ? single.ageGroup !== 'University' &&
                  single.ageGroup !== 'Staff'
                  ? 'Boys'
                  : 'Men'
                : single.ageGroup !== 'University' &&
                  single.ageGroup !== 'Staff'
            ...
            paymentSlip:
              single.paymentSlip != null ? single.paymentSlip : 'N/A',
          };
          await databaseWrapper.createAndUpdate(
            { name: 'single', data: singleData },
            {
              name: 'player',
              data: { pastPerformanceSingle: single.pastPerformance },
              filterField: 'player',
            },
            res
          );
